...
TEST(xpath_variables_set_operations)
{
	...
	CHECK(set.add(STR("var2"), xpath_type_node_set) == nullptr);

	CHECK(set.get(STR("var1")) == v1);
	CHECK(set.get(STR("var2")) == v2);
	CHECK(set.get(STR("var")) == nullptr);
	CHECK(set.get(STR("var11")) == nullptr);

	CHECK(static_cast<const xpath_variable_set&>(set).get(STR("var1")) == v1);
	CHECK(static_cast<const xpath_variable_set&>(set).get(STR("var3")) == nullptr);
}
